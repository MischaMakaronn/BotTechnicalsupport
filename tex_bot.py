...
def create_admin_addprog_keyb(dct):  # функция для создания клавиатуры для добавления программы в БД, принимает словарь
    ...
    for key, value in dct.items():
        AdminAddProg_inline_keyb.add(InlineKeyboardButton(f"{key}. {value[0]}", callback_data=f"admin_add_new_prog:add_prog_{key}"),
...
def admin_management(message):
    ...
    with conn:
        row = [i[1] for i in conn.execute("SELECT * FROM Admins WHERE position = 'admin lvl2' OR position = 'admin lvl1'")]
...
def query_handler(call):
    ...
    if call.data.split(':')[1] == "txt1":
        ...
        [Proga_inline_keyb.add(InlineKeyboardButton(key, callback_data=f"{key}:{value}")) for key, value in column_dict.items()]
        Proga_inline_keyb.add(InlineKeyboardButton("Вернуться в меню", callback_data="menu:b1"))
        bot.edit_message_text("Выберите программу: ", call.message.chat.id ,call.message.message_id, reply_markup=Proga_inline_keyb)
    if call.data.split(':')[1] == "b1" :
        bot.edit_message_text("Что Вас интересует?", call.message.chat.id, call.message.message_id, reply_markup=Main_inline_keyb)
    ...
    if call.data.split(':')[0] in column_dict:
        ...
        [Sub_inline_keyb.add(InlineKeyboardButton(key, callback_data=f"{key}:{value}")) for key, value in column_dict.items() if value == call.data.split(':')[1][1:]]#тут надо выкинуть карточку
        ...
        bot.edit_message_text("Чем я могу помочь?",call.message.chat.id,call.message.message_id, reply_markup=Sub_inline_keyb)
    ...
    if call.data.split(':')[1] == "pushreg":
        ...
        bot.edit_message_text("Чтобы пользоваться чат-ботом, нужно пройти регистрацию.", call.message.chat.id, call.message.message_id,
    ...
    if call.data.split(':')[1] == "reg":
        with conn:
            conn.execute("INSERT OR IGNORE INTO Clients (name, phone_number, adress, telegram_id) values(?, ?, ?, ?)", (reg_name, reg_phone_number, reg_adress, user_telegram_id))
        conn.commit()
        bot.answer_callback_query(call.id)  # подтверждаем нажатие
        bot.edit_message_text("Заполните все поля формы и сохраните данные:",call.message.chat.id,call.message.message_id,  reply_markup=create_registration_keyb())
    ...
    if call.data.split(':')[0] == "accept":
        ...
        conn.commit()
        print("Сохранено")
        bot.send_message(call.message.chat.id, f"Данные профиля успешно сохранены.")
